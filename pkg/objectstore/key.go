...
// BuildObjectKey 生成统一的 MinIO 对象 key：documents/{user_id}/{kb_id}/{task_id}/{safe_file_name}。
// 文件名会被净化，防止路径穿越与任意 key 注入。
func BuildObjectKey(userID, kbID, taskID, fileName string) string {
	return path.Join(ObjectKeyPrefix, userID, kbID, taskID, sanitizeFileName(fileName))
}
