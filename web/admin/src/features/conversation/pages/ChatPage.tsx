...
function ChatPageContent({ kbId, conversationId }: { kbId: string; conversationId?: string }) {
  ...
  const messageArea = (
    <Stack sx={{ flex: 1, minHeight: 0 }}>
      ...
      <MessageList messages={messages} streamingAnswer={submitting && !resumingRun ? runState.answer : ''} agentRunState={runState} agentRunId={activeRunId} agentRunStates={allRunStates} pendingRunIds={pendingRunIds} retryingMessageId={retryingMessageId} resumingRun={resumingRun} emptyComposer={empty ? composer : undefined} onSuggestion={setDraft} onRetry={retry} onSwitchVersion={switchVersion} />
    </Stack>
  );
