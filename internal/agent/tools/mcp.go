...
// networkRequired 标记该工具是否受知识库"联网"开关约束（本地 stdio 工具通常无需联网）。
func NewMCPReadOnlyTool(client internalmcp.MCPClient, target internalmcp.MCPServerTarget, metadata internalmcp.MCPServerTool, serverID, toolID string, enabled bool, networkRequired bool, timeout time.Duration) *MCPReadOnlyTool {
	return &MCPReadOnlyTool{client: client, target: target, metadata: metadata, serverID: serverID, toolID: toolID, enabled: enabled, networkRequired: networkRequired, timeout: timeout}
}
...
func (t *MCPReadOnlyTool) Spec() contracts.ToolSpec {
	...
	return contracts.ToolSpec{Name: compositeName, Description: t.metadata.Description, InputSchema: t.metadata.InputSchema, Type: contracts.ToolTypeMCP, ReadOnly: true, Enabled: t.enabled, SourceID: t.serverID, MCPToolID: t.toolID, NetworkRequired: t.networkRequired, Timeout: t.timeout, MaxCalls: 10}
}
