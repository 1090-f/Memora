...
func buildDocumentProcessService(cfg *config.Config, store *objectstore.Client, tasks repository.ImportTaskRepository, docs repository.DocumentRepository, kbs repository.KnowledgeBaseRepository, chunks repository.DocumentChunkRepository, vectors repository.VectorRepository, embeddings service.DocumentEmbeddingResolver, previewScheduler previewservice.Scheduler, processingEvents repository.DocumentProcessingEventRepository) (service.DocumentProcessService, error) {
	...
	return service.NewDocumentProcessService(tasks, docs, kbs, chunks, vectors, documentPipeline, embeddings, store, previewScheduler, processingEvents), nil
}
