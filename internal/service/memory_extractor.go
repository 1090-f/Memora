...
func (e *memoryExtractor) extractAndStore(ctx context.Context, agentContext contracts.AgentContext, answer string) error {
	// 1. 调用LLM提取候选记忆
	items, err := e.extractCandidates(ctx, agentContext.Query, answer)
	if err != nil {
		return fmt.Errorf("extract candidates: %w", err)
	}

	if len(items) == 0 {
		logger.Info("未提取到候选记忆",
			zap.String("query", agentContext.Query),
		)
		return nil
	}
...
func (e *memoryExtractor) extractCandidates(ctx context.Context, query, answer string) ([]contracts.MemoryItem, error) {
	// 构建Prompt
	...
	// 获取ChatModel
	model, err := e.modelFactory.GetChatModel(ctx, "")
	if err != nil {
